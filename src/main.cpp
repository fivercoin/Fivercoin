...
static const int64_t nTargetTimespan = 20 * 60;  // Fiver - every 20mins
unsigned int nTargetSpacing = 1 * 55; // Fiver - 55 secs
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 5 * 60 * 60; // Fiver - 5 hours
unsigned int nStakeMaxAge = 5 * 60 * 60; // Fiver - 5 hours
unsigned int nModifierInterval = 10 * 60; // Fiver - time to elapse before new modifier is computed
...
const string strMessageMagic = "Fiver Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, unsigned int nHeight)
{
    ...
    if (nHeight > 5000){
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); //5% Per Year
    }else if (nHeight > 2555){
        nSubsidy = 55 * COIN;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Fiver", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "April 23rd 2015 NYT Yankees Quest on Page 1";
        CTransaction txNew;
        txNew.nTime = 1429787613;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
